...
if len(sys.argv) != 6:
    print("Usage: python generate_data.py n_procs n_rows n_cols output_dir n_stragglers")
    sys.exit(0)

#np.random.seed(0)
n_procs, n_rows, n_cols, output_dir, n_stragglers  = [x for x in sys.argv[1:]]
n_procs, n_cols, n_rows, n_stragglers = int(n_procs), int(n_cols), int(n_rows), int(n_stragglers)
